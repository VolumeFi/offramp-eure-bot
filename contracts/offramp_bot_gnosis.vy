...
def swap(receiver: address, amount: uint256, expected: uint256, deposit_id: uint256) -> uint256:
    assert msg.sender == self.compass_evm and convert(slice(msg.data, 132, 32), bytes32) == self.paloma and len(msg.data) == 164, "Unauthorized"
    ...
    if amount < _fee:
        ...
        log Swapped(receiver, _amount, deposit_id)
        return ret
